...
int perform_iterator(rockskv::DB *db, int count, uint8_t klen, uint32_t vlen, char *keys, int key_mode, int it_nums) {
  ...
  int start_key = 0 * count;
  for (int i = start_key; i < start_key + it_nums; i++) {
    //int seed = rdn.Next()%count;
    int seed = i%100000;
    memset(value, 0, vlen);
    //sprintf(key, "%0*d", klen - 1, i);
...
void do_io(int id, rockskv::DB *db, int count, uint8_t klen, uint32_t vlen, int op_type, char *keys, int rd_nums, int key_mode, int key_offset, std::unordered_map<int, int>& rd_lat) {
  ...
  case ITERATOR_OP:
  //   //perform_insertion(id, cont_hd, count, klen, vlen);
    perform_iterator(db, count, klen, vlen, keys, key_mode, rd_nums);
...
int main(int argc, char *argv[]) {
  ...
  if (op_type == WRITE_OP) total_ops = num_ios*t;
  else if (op_type == READ_OP) total_ops = num_rds*t;
  ...
  if(op_type == READ_OP) {
    ...
    fprintf(stdout, "Total time of get %.2f sec; Throughput %.2f ops/sec\n", sec, (double) num_rds * t /sec );
  }
  if(op_type == WRITE_OP) {
    clock_gettime(CLOCK_REALTIME, &t2);
    unsigned long long start, end;
    ...
    double sec = (double)(end - start) / 1000000000L;
    fprintf(stdout, "Total time of put %.2f sec; Throughput %.2f ops/sec\n", sec, (double) num_ios * t /sec );
  }
  if(op_type == ITERATOR_OP) {
    ...
    double sec = (double)(end - start) / 1000000000L;
    fprintf(stdout, "Total time of seek %.2f sec; Throughput %.2f ops/sec\n", sec, (double) num_rds * t /sec );
  }
